...
DEPEND = Makefile \
	...
	imprimir.mac \
	defines.h \
	gdt.c gdt.h pic.c pic.h idt.c idt.h isr.h tss.c tss.h mmu.c mmu.h debug.c debug.h\
...
TASKSBINS=$(TASKIDLE) $(TASKAS) $(TASKAC) $(TASKBS) $(TASKBC)

CODESIZE=60928    # = 65536 - 0x1200 = 64KB - 0x1200
TASKSSIZE=20480   # = 1 * (4096) + 2 * (4096) + 2 * (4096) = idle + tareas
KERNELSIZE=81408  # = CODESIZE + TASKSSIZE
...
taskAS.tsko: tareaAS.c
	$(CC) $(CFLAGS) -c -o $@ $^
taskAC.tsko: tareaAC.c
	$(CC) $(CFLAGS) -c -o $@ $^
taskBS.tsko: tareaBS.c
	$(CC) $(CFLAGS) -c -o $@ $^
...
tasks: $(TASKSBINS)
	...
	$(QUIET) dd if=$(TASKAS)   bs=1 count=4K of=$(TASKS) seek=0K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKAC)   bs=1 count=4K of=$(TASKS) seek=4K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKBS)   bs=1 count=4K of=$(TASKS) seek=8K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKBC)   bs=1 count=4K of=$(TASKS) seek=12K status=noxfer > /dev/null 2>&1
